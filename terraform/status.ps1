...
Get-EC2Instance -Filter @( @{ name='tag:Name'; values="iac*" } ) | select -Expand Instances | ft @{N='Name';E={($_.Tags | ? {$_.Key -eq 'Name'}).Value}}, LaunchTime, Platform, InstanceType, PublicIpAddress | Out-Host