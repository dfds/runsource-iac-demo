terraform {
  source = "./modules/iac-demo"
}

remote_state {
  backend = "s3"
  config = {
    encrypt        = true
    bucket         = "raras-sandbox-state"
    key            = "iac-demo/terraform.tfstate"
    region         = "eu-central-1"
    dynamodb_table = "terraform-locks"
  }
}

inputs = {

  aws_region             = "eu-west-1"
  workload_dns_zone_name = "raras.dfds.cloud"

  ad_name     = "iac.raras.dfds.cloud"

  ec2_public_key = "ssh-rsa  ec2@sandbox"

  server1_name                   = "jumphost"
  server1_windows_server_version = "2019"
  server1_instance_type          = "t3.medium"

  server2_name                   = "server"
  server2_windows_server_version = "2016"
  server2_instance_type          = "t3.medium"

}
